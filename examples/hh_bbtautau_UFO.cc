...
int main(int argc, char** argv) {
    ...
    ConfigurationReader configuration("../examples/hh_bbtautau_UFO.lua");
    ...
    T->SetBranchAddress("gen_target",&gen_target);
...
/*                if (memWeight != memWeight) {
    ...
    outFile << ",memSigWeight, memSigWeight_Error, new_gen_target\n";
    for (int i = 0; i < outputs.size(); ++i) {
        outFile << outputs[i][0] << "," << outputs[i][1] << "," << outputs[i][2] << "," << outputs[i][3] <<"\n";
    }
    outFile.close();
    delete f;
    return 0;
